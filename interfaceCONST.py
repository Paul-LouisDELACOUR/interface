...
def update_output(search, q8p1):
    rating = 'Communication with the client score'
    if(q8p1 == '1'):
...
app.config['suppress_callback_exceptions']=True
@app.callback(
    [Output('datatable-q8', component_property = 'data') , Output('datatable-q8', 'columns')],
    [Input('search_8', 'n_clicks')],
    [State('q8p1', 'value')]
    )
def update_output(search, q8p1):
    review_scores = 'review_scores_communication'
    if(q8p1 == '1') : review_scores = 'review_scores_rating'
    elif(q8p1 == '2') : review_scores = 'review_scores_communication'
    elif(q8p1 == '3') : review_scores = 'review_scores_value'
    elif(q8p1 == '4') : review_scores = 'review_scores_checkin'
    elif(q8p1 == '5') : review_scores = 'review_scores_cleanliness'
    elif(q8p1 == '6') : review_scores = 'review_scores_accuracy'
    elif(q8p1 == '7') : review_scores = 'review_scores_location'
        ...
        if table_set[i] in checklist :
            ...
            for j in range(len(columns_list[i])):
                ...
                if j < (len(columns_list[i]) - 1):
                    string_where += ' OR '
            string_prefix = 'SELECT * FROM '+ table_set[i] + string_where + 'FETCH FIRST 5 ROWS ONLY'
